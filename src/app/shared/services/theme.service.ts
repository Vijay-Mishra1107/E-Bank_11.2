...
export class ThemeService {
  ...
  public WiproBankThemes :ITheme[]  = [{
    "name": "WiproBank-dark-purple",
    "baseColor": "#27458f",
    "isActive": false
  }, {
    "name": "WiproBank-dark-pink",
    "baseColor": "#e91e63",
    "isActive": false
  }, {
    "name": "WiproBank-blue",
  ...
  applyMatTheme( themeName: string) {
    ...
    this.activatedTheme = this.WiproBankThemes.find(t => t.name === themeName); 
  ...
  flipActiveFlag(themeName:string) {
    this.WiproBankThemes.forEach((t) => {
      t.isActive = false;
      if(t.name === themeName) {
        t.isActive = true;
